...
void SoloQueue::AddPlayer(Player* player)
{
    if (CheckRequirements(player))
    {
        if (ArenaTeam* at = sArenaTeamMgr->GetArenaTeamById(player->GetArenaTeamId(ArenaTeam::GetSlotByType(ARENA_TEAM_5v5))))
        {
		  ...
		  //                    queuedHealers.insert(std::pair<uint32/* mmr */, SoloQueueInfo*>(soloInfo->ArenaMatchmakerRating, soloInfo));
                  //  allPlayersInQueue.insert(std::pair<uint64/* mmr */, SoloQueueInfo*>(player->GetGUID(), soloInfo));
		  ...
		  // queuedMelees.insert(std::pair<uint32/* mmr */, SoloQueueInfo*>(soloInfo->ArenaMatchmakerRating, soloInfo));
                  //  allPlayersInQueue.insert(std::pair<uint64/* mmr */, SoloQueueInfo*>(player->GetGUID(), soloInfo));
		  queuedMelees.emplace(soloInfo->ArenaMatchmakerRating, soloInfo);
		  allPlayersInQueue.emplace(player->GetGUID(), soloInfo);
		  break;
                case TALENT_CAT_RANGE:
                  //  queuedRanges.insert(std::pair<uint32/* mmr */, SoloQueueInfo*>(soloInfo->ArenaMatchmakerRating, soloInfo));
                  //  allPlayersInQueue.insert(std::pair<uint64/* mmr */, SoloQueueInfo*>(player->GetGUID(), soloInfo));
