    ...
    while (i != m_InstancedMaps.end())
    {
        ...
        if (crash)
        {
            ...
            for (PlayerList::const_iterator itr = pList.begin(); itr != pList.end(); ++itr)
            {
                ...
                player->GetSession()->KickPlayer("anticrash");
            ...
            // i have no id why this this is never show in world !!!
            sWorld->SendServerMessage(SERVER_MSG_STRING, buf.c_str());
            TC_LOG_ERROR("anticrash", "Crash map done on %s (ID %u)", i->second->GetMapName(), i->second->GetInstanceId());
        ...
        else if (killed)
        {
            ...
            TC_LOG_ERROR("server.loading", "KillInstance done on %s (ID %u)", i->second->GetMapName(), i->second->GetInstanceId());
            ...
            if (sigsetjmp(env,1))
            {
                ...
                in_handler = false;
                TC_LOG_ERROR("anticrash", "warning: crash during instance cleanup (handled)");
