...
bool WorldSession::HandleMovementInfo(MovementInfo &movementInfo, const uint16 opcode, const size_t packSize, Unit *mover)
{
    ...
    // Don't process any Movement opcodes when movement is controlled by server.
    //    if (plrMover && plrMover->GetMotionMaster()->GetCurrentMovementGeneratorType() && !plrMover->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_TAXI_FLIGHT))
    //    return false;
