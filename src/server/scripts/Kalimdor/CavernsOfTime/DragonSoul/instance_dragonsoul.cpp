            ...
            void OnCreatureCreate(Creature* creature) override
            {
                ...
                switch (creature->GetEntry())
                {
                    ...
                    case NPC_EARTHEN_SOLDIER:
                        npcGroups.insert(std::pair<uint32, uint64>(DATA_MORCHOK, creature->GetGUID()));
                    ...
                    case NPC_ASPECT_4_OUTRO:
                        if (!IsDone(DATA_MADNESS_OF_DEATHWING))
                            creature->SetVisible(false);
                        npcGroups.insert(std::pair<uint32, uint64>(DATA_MADNESS_OF_DEATHWING, creature->GetGUID()));
                        break;
                    ...
                    case NPC_NETHESTRASZ:
                        if (!IsDone(DATA_YORSAHJ) && !IsDone(DATA_WARLORD_ZONOZZ) && LfrId != LFR_PART_TWO)
                            creature->SetVisible(false);
                        npcGroups.insert(std::pair<uint32, uint64>(DATA_YORSAHJ, creature->GetGUID()));
                        break;
                    case NPC_TRAVEL_TO_THE_MAELSTROM:
                        if (!IsDone(DATA_SPINE_OF_DEATHWING))
                            creature->SetVisible(false);
                        npcGroups.insert(std::pair<uint32, uint64>(DATA_SPINE_OF_DEATHWING, creature->GetGUID()));
                        break;
                    case NPC_MORCHOK:
