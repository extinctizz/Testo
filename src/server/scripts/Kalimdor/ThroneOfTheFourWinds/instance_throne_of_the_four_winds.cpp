...
public:
    ...
    class FallingEvent : public BasicEvent
    {
    public:
        FallingEvent(Creature* trash, uint64 player) : _trash(trash), _playerGUID(player)
        {
        }
        ...
        bool Execute(uint64 execTime, uint32 /*diff*/)
        {
            if (Unit *_player = Unit::GetUnit(*_trash, _playerGUID))
                if (_player->GetMapId() == _trash->GetMapId())
                    if (_player->IsInWorld())
                        if (_player->GetMap() != NULL)
                        {
                            if (Vehicle *veh = _trash->GetVehicleKit())
                                veh->RemoveAllPassengers();
                            _player->RemoveAurasDueToSpell(85269);
                            // this hack because player movement is falling every time, and z position isn't updated, whenever player is on a flying vehicle...
                            _player->NearTeleportTo(_player->GetPositionX(), _player->GetPositionY(), 200.0f, 0.0f);
                            _player->UpdatePosition(_player->GetPositionX(), _player->GetPositionY(), 200.0f, true);
                            _player->ToPlayer()->SetViewpoint(_trash, false);
                            _trash->CastSpell(_player, 85274, true);
                            if (InstanceScript* instance = _player->GetInstanceScript())
                                if (instance && (instance->GetBossState(DATA_ALAKIR_EVENT) == DONE || instance->GetBossState(DATA_ALAKIR_EVENT) == DONE_HM))
                                    _player->AddAura(82724, _player);
                        }
            _trash->ToCreature()->DespawnOrUnsummon(200);
            return true;
        }
    private:
        Creature* _trash;
