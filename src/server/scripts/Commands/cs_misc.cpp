...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        static std::vector<ChatCommand> sendCommandTable =
        {
            ...
            { "itemset",            SEC_ADMINISTRATOR,      true,  &HandleSendItemsetCommand,           "" },
        };
        ...
        static std::vector<ChatCommand> commandTable =
        {
            ...
            { "xp",                 SEC_PLAYER,             false, HandleXPRateCommand,                 "" },
            { "arena",              SEC_PLAYER,             false, HandleArenaCommand,                  "" },
            { "synchronized",       SEC_PLAYER,             false, HandleSynchroniseAchievementCommand, "" }
        };
        return commandTable;
    }
    ...
    */
    ...
    static bool HandleSynchroniseAchievementCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        if (!player)
            return false;
        sObjectMgr->_migration->FillWotlkAchievements(player);

        std::string mess = "Felicitation, vos hauts faits ont bien ete synchronises. ";
        handler->PSendSysMessage(mess.c_str());
        return true;
    }
};
