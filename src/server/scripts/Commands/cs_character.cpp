...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        static std::vector<ChatCommand> commandTable =
        {
            ...
            { "migration",      SEC_ADMINISTRATOR,  true,  &HandleMigrationCommand,                ""},
        };
    ...
    */
    ...
    static bool HandleMigrationCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        if (target->GetWotlkGUID() <= 0)
        {
            if (QueryResult result = WotlkCharacterDatabase.PQuery("SELECT guid from characters WHERE cataGUID = %u", target->GetGUIDLow()))
            {
                do
                {
                    Field* fields = result->Fetch();
                    uint32 wotlkGUID =  fields[0].GetUInt32();
                    CharacterDatabase.PExecute("UPDATE characters SET WotlkGUID =%u, NeedSynchronisation = 1 WHERE guid = %u", wotlkGUID, target->GetGUIDLow());
                    handler->PSendSysMessage("Migration corrompue detecte et corrigee");
                }
                while (result->NextRow());
            }
            else
                handler->PSendSysMessage("Aucune migration effectue par le joueur");
        }
        else
            handler->PSendSysMessage("Migration deja effectue");
        return true;
    }
